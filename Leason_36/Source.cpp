...
    */
    ...
    // Упаковка------------------------------------------------------------
    // Добавить проверку правильности ввода данных
#if 0
    ...
    //date = date << 4;// место для месяца
    date <<= 4;// место для месяца

    _itoa_s(date, str, 2);
    cout << setw(12) << str << endl;

    date |= d; // устанавливаем биты месяца
    ...
    date <<= 11;// место для года

    _itoa_s(date, str, 2);
    cout << setw(12) << str << endl;

    _itoa_s(d, str, 2);
    cout << setw(19) << str << endl;

    date |= d; // устанавливаем биты года

    _itoa_s(date, str, 2);
    cout << setw(13) << str << endl;
    ...
    date |= d; // устанавливаем биты часов
    ...
    //5(день)   4(месяц)   11(год)  5(часы)   6(минут)
    ...
    int m1 = 63; //0x3F
    int m2 = 31; // 0x1F
    int m3 = 0x7FF;// 2047 - 11 единиц в первых битах
    int m4 = 0xF;//15   
...
Области применения битовых операций:
    ...
    - Повышение производительности программ —
      битовые операции могут выполняться одновременно со всеми разрядами.
...
Примеры программирования на уровне отдельных двоичных разрядов:
·        упаковка данных, архивирование.
        Естественно уплотнение данных производить с точностью не до байта, а до бита;

·        алгоритмы сортировок и структуры данных, использующие элементы
          внутреннего представления хранимых данных: поразрядные сортировки, кодовые деревья;

·        моделирование машинной арифметики.
        Если нас не устраивает точность представления данных, принятая в компьютере,
        мы сможем программно смоделировать данные более высокой точности. П
        ри этом можно пользоваться разными формами их представления, в том числе принятыми двоичными форматами данных;

·        работа с растрами, битовыми картами (bitmap).
        Например, для контроля свободного пространства на диске файловая система
        часто использует карту памяти – массив байтов,
        каждый разряд которого определяется состояние отдельного блока: 0 – свободен, 1 – занят.
